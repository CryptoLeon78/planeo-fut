...
export default defineAuthedTool({
  ...
  description:
    "Bring back a practice, session or microcycle that was soft deleted, together with its version history.",
  inputSchema: {
  ...
  handler: async ({ entity, recordId }, ctx, userId) => {
    ...
    return toolSuccess(
      `Restored ${entity} “${String((data as unknown as Record<string, unknown>).name)}”.`,
      {
        record: data,
      },
    );
