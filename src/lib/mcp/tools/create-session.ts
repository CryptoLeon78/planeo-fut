...
export default defineAuthedTool({
  ...
  description:
    "Create a football training session for the signed-in coach with its date, objective and planned load.",
  inputSchema: {
    ...
    durationMinutes: z
      ...
      .max(240)
      .optional()
      .describe("Total planned duration in minutes (15-240)."),
    intensity: intensityEnum
      .optional()
      .describe("Planned session intensity: baja, media, alta or muy_alta."),
  ...
  handler: async (input, ctx, userId) => {
    ...
    return toolSuccess(`Created session “${data.name}” (version ${version}).`, {
      session: data,
      version,
    });
  },
