    ...
    <Card className="space-y-4 p-5 print:hidden">
      ...
      <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((n) => (
              <button
                key={n}
                type="button"
                aria-label={`Valorar con ${n} de 5`}
                onClick={() => save({ rating: n })}
              ...
              >
                <Star
                  className="h-4 w-4"
                  fill={(evaluation.rating ?? 0) >= n ? "currentColor" : "none"}
                />
              </button>
            ))}
          </div>
        ...
        <div className="space-y-1.5">
          ...
          <Select
            value={evaluation.intensity_perceived ?? undefined}
            onValueChange={(v) => save({ intensity_perceived: v })}
          >
            <SelectTrigger>
        ...
        <div className="flex items-center justify-between rounded-md border border-border bg-secondary/40 px-3 py-2 md:col-span-2">
          ...
          <Switch
            checked={!!evaluation.objectives_met}
            onCheckedChange={(v) => save({ objectives_met: v })}
          />
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Textarea
            rows={3}
            defaultValue={evaluation.what_worked ?? ""}
            onBlur={(e) => {
              if (e.target.value !== (evaluation.what_worked ?? ""))
                save({ what_worked: e.target.value });
            }}
          />
        </div>
        <div className="space-y-1.5">
          ...
          <Textarea
            rows={3}
            defaultValue={evaluation.what_to_improve ?? ""}
            onBlur={(e) => {
              if (e.target.value !== (evaluation.what_to_improve ?? ""))
                save({ what_to_improve: e.target.value });
            }}
          />
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          rows={2}
          defaultValue={evaluation.player_notes ?? ""}
          onBlur={(e) => {
            if (e.target.value !== (evaluation.player_notes ?? ""))
              save({ player_notes: e.target.value });
          }}
        />
      </div>
